  ...
  const reload = async () => {
    if (!supabase) return
      ...
      setData({
        ...
        dealTrips: (trips.data ?? []).map((trip) => ({ id: trip.id, dealId: trip.deal_id, transporterId: trip.transporter_id, truckNumber: trip.truck_number || '', driverPhone: trip.driver_phone || '', freight: rupees(trip.freight_paise), assignedDate: trip.assigned_at?.slice(0, 10) ?? '', notes: trip.notes })),
        ...
        transporters: (transporters.data ?? []).map((t) => ({ id: t.id, name: t.name, phone: t.phone, routes: t.routes ?? t.notes ?? '', reliability: t.reliability[0].toUpperCase() + t.reliability.slice(1) })),
        trucks: (trucks.data ?? []).map((t) => ({ id: t.id, number: t.registration_number, transporterId: t.transporter_id, capacity: t.capacity_qtl ? String(t.capacity_qtl) : '', driverPhone: t.driver_phone ?? '' })),
