...
export function useCloudData() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if ([parties, deals, payments, obligations, transporters, trucks, offers, requirements, todos].some((result) => result.error)) { setLoaded(true); return }
      setData({
        ...
        deals: (deals.data ?? []).map((d) => ({ id: d.id, dealDate: d.deal_date, dailySerial: d.daily_serial, monthlySerial: d.monthly_serial, buyerId: d.buyer_party_id, sellerId: d.seller_party_id, commodity: d.commodity, quantity: Number(d.quantity_qtl), rate: rupees(d.rate_per_qtl_paise), route: d.route, stage: fromDbStage(d.stage), truck: '', expectedPayment: '', cancelledReason: d.cancelled_reason ?? undefined })),
        dealTrips: [],
